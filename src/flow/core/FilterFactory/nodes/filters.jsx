...
export const filters = [
    ...
    { value: "publication_year", label: "Publication Year", groupLabel:"Core", group:"core",inputType:"year" },
    ...
    { value: "has_fulltext", label: "Has FullText", groupLabel:"Misc",group:"misc",inputType:"boolean" },
...
const allfilterTypes = ['Search Fields','Core','Authorship','Publication','Concepts','Open Access','Citations', 'Identifiers','APC', 'Correspondance', 'Funding', undefined, 'Publication Dates', 'Misc'];
...
// Group filters
const groupedFilters = Object.values(filterCollection.items.reduce((acc, item) => {
  if (!acc[item.group]) {
    acc[item.group] = {
      filterType: item.group,
      filterTypeLabel: item.groupLabel,
      items: []
    };
  }
...
export const filterTypes = groupedFilters.sort((a, b) => {
  // Handle null/undefined filterTypes
  if (!a.groupLabel && !b.groupLabel) return 0;
  if (!a.groupLabel) return 1;
  if (!b.groupLabel) return -1;

  const indexA = allfilterTypes.indexOf(a.groupLabel);
  const indexB = allfilterTypes.indexOf(b.groupLabel);

  // If both items aren't in allfilterTypes, maintain their relative order
  if (indexA === -1 && indexB === -1) return 0;
  
  // Put unknown filterTypes at the end
